...
| **会话开关** | 悬浮条上的 [×] 关闭按钮 | 临时关闭当前会话的语音，悬浮条隐藏，录音停止。页面刷新后恢复 |
| **快捷键** | 待定（如 `Ctrl+Shift+V`） | 快速切换悬浮条显示/隐藏 |
...
- 会话开关：存储在浏览器 `localStorage`（`voice.sessionEnabled`），仅影响当前浏览器
